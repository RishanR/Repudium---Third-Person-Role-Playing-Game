	...
	public class RPGCharacterMovementControllerFREE : SuperStateMachine{
		...
		*/
		...
		protected override void LateGlobalSuperUpdate(){
			...
			transform.position += currentVelocity * rpgCharacterController.movementAnimationSpeed * superCharacterController.deltaTime;
			...
			if(!useMeshNav && !rpgCharacterController.isDead && canMove){
				if(currentVelocity.magnitude > 0 && rpgCharacterInputController.HasMoveInput()){
					isMoving = true;
					animator.SetBool("Moving", true);
					animator.SetFloat("Velocity Z", currentVelocity.magnitude * rpgCharacterController.movementAnimationSpeed);
				}
				else{
					isMoving = false;
		...
		void Idle_SuperUpdate(){
			...
			if(rpgCharacterInputController.HasMoveInput()){
